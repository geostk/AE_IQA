...
# Processing the samples
    ...
    # No distortion
        ...
        x_dist[L*idx] = I_dist
    # Distortion level 1
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=0.5)))
        x_clean[L*idx+1] = I_clean
        x_dist[L*idx+1] = I_dist/255.
    # Distortion level 2
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=1)))
        x_clean[L*idx+2] = I_clean
        x_dist[L*idx+2] = I_dist/255.
    # Distortion level 3
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=1.5)))
        x_clean[L*idx+3] = I_clean
        x_dist[L*idx+3] = I_dist/255.
    # Distortion level 4
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=2)))
        x_clean[L*idx+4] = I_clean
        x_dist[L*idx+4] = I_dist/255.
    # Distortion level 5
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=2.5)))
        x_clean[L*idx+5] = I_clean
        x_dist[L*idx+5] = I_dist/255.
    # Distortion level 6
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=3)))
        x_clean[L*idx+6] = I_clean
        x_dist[L*idx+6] = I_dist/255.
    # Distortion level 7
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=3.5)))
        x_clean[L*idx+7] = I_clean
        x_dist[L*idx+7] = I_dist/255.
    # Distortion level 8
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=4)))
        x_clean[L*idx+8] = I_clean
        x_dist[L*idx+8] = I_dist/255.
    # Distortion level 9
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=4.5)))
        x_clean[L*idx+9] = I_clean
        x_dist[L*idx+9] = I_dist/255.
    # Distortion level 10
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=5)))
        x_clean[L*idx+10] = I_clean
        x_dist[L*idx+10] = I_dist/255.
    # Distortion level 11
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=5.5)))
        x_clean[L*idx+11] = I_clean
        x_dist[L*idx+11] = I_dist/255.
    # Distortion level 12
        I_dist = np.array(im.filter(ImageFilter.GaussianBlur(radius=6)))
        x_clean[L*idx+12] = I_clean
        x_dist[L*idx+12] = I_dist/255.
    # Display
        if (idx+1)%5000 == 0:
            print '%d/%d images done!' % (idx+1, num_samples)
