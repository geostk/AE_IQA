...
for i in range(L):
    ...
    dist = Conv2D(F[i], (3, 3), activation='relu', padding='same', strides=(1, 1))(dist)

dist_encoder = Model(inputs=[dist_enc_inp], outputs=[dist])
dist_encoder.compile(optimizer='adadelta', loss='mean_squared_error')

# Decoder
inp = Input(shape = (None, None, F[L-1]))
dec = inp
for i in range(-1, L-1)[::-1]:
    u = UpSampling2D((2, 2))(dec)
    if i != -1:
        dec = Conv2D(F[i], (3, 3), activation='relu', padding='same', strides=(1, 1))(u)
        dec = Conv2D(F[i], (3, 3), activation='relu', padding='same', strides=(1, 1))(dec)
    else:
        recon = Conv2D(3, (3, 3), activation='relu', padding='same')(u)

decoder = Model(inputs=inp, outputs=[recon])
...
noise = decoder(dist_encoder(dist_input))
dist_pred = add([clean_input, dist_input])
...
vis_clean = utils.visualize(x_test, [32, 32], [1, 1], [5, 5], color=1, channels='last')
vis_dist = utils.visualize(x_test_noisy, [32, 32], [1, 1], [5, 5], color=1, channels='last')
vis_dist_pred = utils.visualize(decoded_imgs, [32, 32], [1, 1], [5, 5], color=1, channels='last')

plt.figure(figsize=(15, 15))
plt.subplot(1, 3, 1), plt.imshow(vis_clean)
plt.subplot(1, 3, 2), plt.imshow(vis_dist)
plt.subplot(1, 3, 3), plt.imshow(vis_dist_pred)
plt.show()
