...
clean_encoder = Model(inputs=[clean_enc_inp], outputs=[enc])
clean_encoder.compile(optimizer='adadelta', loss='mean_squared_error')

# Decoder
dec_inp = Input(shape = (None, None, F[L-1]))
...
decoder = Model(inputs=[dec_inp], outputs=[recon])
...
clean_enc = clean_encoder(clean_input)
dist_enc = dist_encoder(dist_input)
corr_enc = add([clean_enc, dist_enc])
dist_pred = decoder([corr_enc])
