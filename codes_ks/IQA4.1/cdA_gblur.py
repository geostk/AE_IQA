...
enc = clean_enc_inp
for i in range(L):
    enc = Conv2D(F[i], (3, 3), activation='relu', padding='same', strides=(2, 2), 
                 kernel_initializer='glorot_uniform', bias_initializer='zeros')(enc)
    encs.append(enc)
...
for i in range(-1, L-1)[::-1]:
    ...
    if i != -1:
        dec = Conv2D(F[i], (3, 3), activation='relu', padding='same',
                     kernel_initializer='glorot_uniform', bias_initializer='zeros')(u)
...
dist = dist_enc_inp
for i in range(L):
    dist = Conv2D(F[i], (3, 3), activation='relu', padding='same', strides=(2, 2),
                  kernel_initializer='glorot_uniform', bias_initializer='zeros')(dist)
    encs.append(dist)

dist_encoder = Model(inputs=[dist_enc_inp], outputs=encs)
...
dist_encs = []
for i in range(L):
    clean_enc = Input(shape=(None, None, F[i]))
    dist_enc = Input(shape=(None, None, F[i]))
    clean_encs.append(clean_enc)
    dist_encs.append(dist_enc)
    
corr_encs = []
for i in range(L):
    corr_enc = add([clean_encs[i], dist_encs[i]])
    corr_encs.append(corr_enc)
    
correction = Model(inputs=clean_encs+dist_encs, outputs=corr_encs)
correction.compile(optimizer='adadelta', loss='mean_squared_error')
...
dist_encs = dist_encoder(dist_input)
corr_encs = correction(clean_encs+dist_encs)
...
vis_clean = utils.visualize(x_test, [32, 32], [1, 1], [13, 13], color=1)
vis_dist = utils.visualize(x_test_noisy, [32, 32], [1, 1], [13, 13], color=1)
vis_clean_filt = utils.visualize(decoded_imgs[0], [32, 32], [1, 1], [13, 13], color=1)
vis_dist_filt = utils.visualize(decoded_imgs[1], [32, 32], [1, 1], [13, 13], color=1)
